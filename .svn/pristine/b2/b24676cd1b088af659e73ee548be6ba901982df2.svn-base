...
//									CYMGFlowWindowClose win = CYMGFlowWindowClose.getInstance(context);
//									win.show();
							}
						}
					});
	...
	public static void callbackResult(final String result,final Context context){
...
//				callback.onFinished(result);
				try {
					...
					int state_code = obj.getInt(CYMGProtocolKeys.STATE_CODE);
					//初始化成功
					if (state_code == CYMGPrompt.CODE_INIT_SUCCESS) {
						initSplash(context,result);
					}else {
						callback.onFinished(result);
					}
	...
	private static void initSplash(final Context context, final String result) {
		((Activity)context).runOnUiThread(new Runnable() {
			
			@Override
			public void run() {
				Intent intent = new Intent(context,CYMGSplashActivity.class);
				intent.putExtra("ScreenOrientation", SettingSPUtil.getLandScape(context));
