...
//			}
	...
	private void intoMGPWordOrderQueryFragment() {
		...
		CYMGCustomerServiceActivity.addFragment(wordOrderQueryFragment);
		clearETData();
	}
	
	private void change2IMFragment() {
		FragmentTransaction fragmentTransaction = mFragmentManager
				.beginTransaction();
		CYMGCustomServiceIMFragment customServiceIMFragment = new CYMGCustomServiceIMFragment();
		customServiceIMFragment.setArguments(mBundle);
		fragmentTransaction.replace(mResource.mgp_onlineserver_root_layout,
				customServiceIMFragment,
				Contants.FragmentTag.ONLINESERVER_IM_FRAGMENT)
				.addToBackStack(
						Contants.FragmentTag.ONLINESERVER_IM_FRAGMENT);
		fragmentTransaction.commit();
		CYMGCustomerServiceActivity.addFragment(customServiceIMFragment);
	}
