...
//			}
		...
		WebChromeClient chromeClient = new WebChromeClient(){
			...
			public void onReceivedTitle(WebView view, String title) {
				mTitleTV.setText(title.length() > 4 ? title.substring(0, 4) : title);
			}
	...
	public class MyInterface{
		...
		@JavascriptInterface 
		public void tonativepage(String key){
			if(key.equals(OPEN_CHARGE_HISTORY)){
				if(CYMGGameParams.getInstance().getHandler() != null){
					//打开充值记录
					Bundle ChargeRecorderBundle = new Bundle();
					ChargeRecorderBundle.putString(CYMGProtocolKeys.UID, CYMGGameParams.getInstance().getHandler().getUID());//设置当前账户uid
					CYMGPlatform.getInstance().chargeRecorder(getActivity(), ChargeRecorderBundle);
				}
			}
		}
