...
public class CYMGHttpRequest {
	...
	public void getPaymentGoodsRequest(final OnRequestListener<GoodsList> listener){
		if(NetWorkUtils.isNetworkConnected(mContext)){
			...
			mClient.get(
					...
					, map, new MyAsyncResponseHandler("getPaymentGoodsRequest"){
				@Override
				public void onStart() {
					listener.onStart();
				}
	...
	public void uploadCapture(String path,String cysid,final OnRequestListener<UploadImgResultBean> listener){
		try {
			...
			mClient.post(HttpContants.UPLOAD_IMG_URL, params, new MyAsyncResponseHandler("uploadCapture"){
	...
	public void shareToGameCircle(String rows,String appkey,String token,String content,final OnRequestListener<Boolean> listener){
		...
		mClient.post(HttpContants.GAME_CIRCLE_SHARE, params,new MyAsyncResponseHandler("shareToGameCircle"){
...
//			params.put("push_info", mBundle.getString(CYMGProtocolKeys.PUSH_INFO));
			...
			mCreateOrderHttp.post(HttpContants.getURL(HttpContants.CREATE_ORDER_FROM_SERVER), params, new MyAsyncResponseHandler("createOrderFromServer"){
				@Override
				public void onStart() {
					listener.onStart();
				}
...
//					mHandler.sendEmptyMessage(PAY_WAY_GET_ORDERID_FAIL);
	 ...
	 * @param listener
	 */
	public void getOrderList(Context ctx,String uid,String pageNo,final OnRequestListener<OrderList> listener){
		MyHttpClient client = new MyHttpClient(ctx);
		RequestParams params = new RequestParams();
		params.put("user_id", uid);
		...
		params.put("status", "");
		client.get(HttpContants.getURL(HttpContants.ORDER_LIST), params, new MyAsyncResponseHandler("getOrderList"){
			...
			public void onSuccess(int statusCode, String content) {
				try {
					...
					listener.onSuccess(statusCode, list);
				} catch (Exception e) {
					log.e(e);
				}
