...
import com.changyou.mgp.sdk.mbi.pay.presenter.OrderPresenter;
 ...
 * @date 2014-1-14
 */
public class CYMGChargeRecorderFragment extends BaseFragment implements OnClickListener,IOrderHandler{
	...
	protected void initData() {
		...
		mOrderPresenter.requestOrderList(getActivity(), mUID, mPageNo, false);
	}
	...
	protected void initEvent() {
		...
		this.mListView.setOnDropDownListener(new OnDropDownListener() {
			...
			public void onDropDown() {
				mPageNo = 1;
				mOrderPresenter.requestOrderList(getActivity(), mUID, mPageNo, true);
			}
		});
	...
	public void onSuccessRequestOrderList(OrderList list,boolean refresh) {
		...
		if (list.getData().isEmpty() && !refresh && !mData.isEmpty()) {
			showLongToast(ResourceUtil.getInstance(getActivity()).getStringId("mgp_feedback_refresh_nodata_text"));
		}
	}
	...
	public void onFailedRequestOrderList(int statusCode, String content,boolean refresh) {
		...
		if(mData.isEmpty()){
			...
			showLongToast(ResourceUtil.getInstance(getActivity()).getStringId("mgp_net_error_hint"));
		}
		if(refresh){
			mListView.onDropDownComplete();
		}
	...
	public void showEmptyView(int size) {
		...
		mNetErrorView.setVisibility(View.GONE);
		mListView.onBottomComplete(size);
	}
