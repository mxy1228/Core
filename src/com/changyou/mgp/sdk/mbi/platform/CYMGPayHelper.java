...
//		this.mTimer = new Timer();
		...
		public void handleMessage(android.os.Message msg) {
			...
			case VALIDATE_SUCCESS:
				Toast.makeText(
						mActivity,
						mActivity.getString(
								PayResource.getInstance(mActivity).mgp_validate_order_success), Toast.LENGTH_SHORT).show();
				break;
			default:
				break;
			}
	...
	public void paySuccess(final String uid, final GoodsItem item,
		...
		CMBILogManager.printEventLog(a, "0", "120052", "");
		this.mActivity = a;
...
//				3 * 1000).show();
	...
	public void payException(int errorCode){
		CMBILogManager.printEventLog(mActivity, "0", "120052", "");
		CYMGCallbackHelper.callbackResult(CYMGCallbackHelper.getCommonResult(errorCode, null),mActivity);
	}
	...
	private void validateOrder(final String uid, final GoodsItem item,
		...
		client.post(HttpContants.getURL(HttpContants.VERIFY_ORDER), params, new MyAsyncResponseHandler(){
			...
			@Override
			public void onStart() {
				CMBILogManager.printEventLog(mActivity, "0", "110053", "");
				mValidating = true;
			}
			
			@Override
			public void onSuccess(int statusCode, String content) {
				CMBILogManager.printEventLog(mActivity, "0", "120053", "");
			...
			if (value != null && !TextUtils.isEmpty(value)) {
				...
				if (b != null) {
					...
					CYMGCallbackHelper.callbackResult(getFileErrorJson(key,
							CYMGPrompt.CODE_EMPTY_FILE),mActivity);
				}
			} else {
				CYMGCallbackHelper.callbackResult(getFileErrorJson(key,
						CYMGPrompt.CODE_EMPTY_PRO_KEY),mActivity);
			}
		} catch (Exception e) {
			log.e(e);
		}
	...
	public void createOrderFromServer(final Bundle bundle, final GoodsItem item) {
		...
		final String channel_id = MetaDataValueUtils.getChannelID(mActivity);
						...
						public void onSuccess(int statusCode, String content) {
							try {
								...
								if(orderSign.equals(md5_two)){
									...
									CYMGChannel channelDo = CYMGChannelHelper.getChannel(mActivity,channelID);
